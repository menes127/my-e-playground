 ...
 * @version $Id: AlwaysTestAfterTimeInMillisCaptchaChannelProcessor.java 2743 2008-03-17 11:06:32Z luke_t $
 */
public class AlwaysTestAfterTimeInMillisCaptchaChannelProcessor
    ...
    boolean isContextValidConcerningHumanity(CaptchaSecurityContext context) {
        if ((System.currentTimeMillis() -
                context.getLastPassedCaptchaDateInMillis()) < getThreshold()) {
            logger.debug(
                "context is valid : current time - last passed captcha date < threshold");

            return true;
        } else {
            logger.debug(
                "context is not valid : current time - last passed captcha date > threshold");
