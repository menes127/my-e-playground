 ...
 * @version $Id: AlwaysTestBelowAverageTimeInMillisBetweenRequestsChannelProcessor.java 2743 2008-03-17 11:06:32Z luke_t $
 */
public class AlwaysTestBelowAverageTimeInMillisBetweenRequestsChannelProcessor
    ...
    boolean isContextValidConcerningHumanity(CaptchaSecurityContext context) {
        ...
        if (context.isHuman() && (average > thresold)) {
            logger.debug(
                "context is valid : average time between requests < threshold && is human");

            return true;
        } else {
            logger.debug(
                "context is not valid : average time between requests > threshold or is not human");
